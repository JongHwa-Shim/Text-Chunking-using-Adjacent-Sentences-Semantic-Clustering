...
## Core Method
- The input text is vectorized into sentences, then a greedy semantic chunking method is performed, starting a new chunk at a point where the cosine similarity between adjacent sentences decreases.
- If the string length of a chunk generated by `cluster_text` exceeds `max_cluster_len`, the chunk is re-vectorized using the splitter & encoder, and then re-clustered by increasing the threshold to create shorter chunks. 
- Therefore, only chunks with string lengths within the range `min_cluster_len` to `max_cluster_len` are registered in the results. 
- Chunks smaller than `min_cluster_len` are ignored (e.g., 2-3 characters) as they are too small. 
- This method filters out extremely short or long chunks from the results, and maintains stable chunk density and size by adjusting both the threshold and length conditions.
